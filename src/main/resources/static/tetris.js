...
var shapeRotateMap = [1,0,3,2,4,6,7,8,5,10,9,12,13,14,11,16,17,18,15]; //도형 회전

var shapeColorArray = [ //도형 색상
    "rgb(199,82,82)", // 분홍
    "rgb(233,174,43)", // 노랑
    "rgb(105,155,55)", // 초록
    "rgb(53,135,145)", // 하늘
...
var tileColor = "rgb(9,17,26)", //배경
    shapeColor,
    wallColor = "rgb(22,41,63)"; //벽 색깔
...
function keyDownEventHandler(e){
    switch(e.keyCode){
        case 37: setTimeout("moveLR(-1)",0); break; //왼쪽 방향키
        case 39: setTimeout("moveLR(1)",0); break; //오른쪽 방향키
        case 38: setTimeout("rotateShape()",0); break; //윗 방향키
        case 40: moveFast(); break; //아래 방향키
...
// 도형 생성
function chooseNextShape(){ //도형 랜덤 생성
    nextShape = parseInt(Math.random() * shapeArray.length);
}

function chooseNextColor(){ //도형 색상 랜덤
...
function checkLine(){
    ...
    for(var i = H-2; i > 1; i--){
        if(isFull(i)){
            removeLine(i);
            i++;
            finalScore += updateScore(plusScore,++combo); //최종 스코어(DB참조)
        }
...
function gameOver(){
    ...
    document.scoreForm.ID.value=document.getElementById('span1').innerHTML;
    document.scoreForm.Score.value=`${score}`;
    
    var scoreForm = document.scoreForm;
	//scoreForm.submit();
